...
- Present: Drizzle schema + migrations for `users`, `teams`, `team_members`, `team_invitations`, `feature_items`; `/auth` route with sign-in/sign-up server actions; shared dashboard layout with overview, settings (profile/email/password), team management, and a DB-backed feature CRUD reference page; invitation acceptance flow; reusable UI primitives; centralized theme system.
...
- Schema: `lib/db/schema.ts` — tables: `users`, `teams`, `team_members`, `team_invitations`, `feature_items`.
- Client: `lib/db/client.ts`.
- Migrations: `drizzle/*.sql` + `drizzle/meta/_journal.json` (`0000` base tables + `0001` feature CRUD table).
